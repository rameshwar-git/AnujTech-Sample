...
public class InterviewUpload extends AppCompatActivity {
    ...
    private void onClick(@NonNull View view) {
        if (view.getId() == R.id.select) {
            ...
            StorageReference upload = storageReference.child("Images").child("Interview");
            upload.child(mFileName.substring(mFileName.length()-7)).putFile(mFileUri)
                    .addOnSuccessListener(taskSnapshot -> taskSnapshot.getMetadata().getReference().getDownloadUrl()
                            .addOnSuccessListener(uri -> {
                                ...
                                databaseReference.child("Interview").child("Clint").child(formattedDateTime).setValue(imageDataModel);
                            })
                    )
